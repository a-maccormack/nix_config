  ...
  config = {
    ...
    environment.systemPackages = with pkgs; [
      ...
      (writeShellScriptBin "install-homelab" ''
        ...
        mkdir -p /mnt/home/mac/docker-compose
        cp "$CONFIG_DIR/hosts/homelab/docker-compose/docker-compose.yml" /mnt/home/mac/docker-compose/
        cp "$CONFIG_DIR/hosts/homelab/docker-compose/.env.example" /mnt/home/mac/docker-compose/.env
        ...
        echo "  3. Reboot and SSH to port 2222 to unlock LUKS:"
        echo -e "     ''${CYAN}ssh -p 2222 -i ~/.ssh/homelab_unlock root@$LOCAL_IP''${NC}"
        echo "  4. After unlock, SSH to the server:"
        echo -e "     ''${CYAN}ssh -i ~/.ssh/homelab mac@$LOCAL_IP''${NC}"
        echo "  5. Set up Tailscale: sudo tailscale up --ssh"
        echo "  6. Edit .env if needed: nano ~/docker-compose/.env"
        echo ""
